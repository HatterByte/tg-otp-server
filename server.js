...
app.post("/telegram-webhook", async (req, res) => {
  ...
  if (text === "/start") {
    await sendMessage(
      chatId,
      "👋 Welcome! Please send your mobile number to register for OTP services by either clicking the button below or manually typing..",
      {
        keyboard: [
    ...
    if (phoneNumber.length === 10) {
      ...
      await sendMessage(
        chatId,
        `✅ Registered! Your phone number (${phoneNumber}) is linked.`
      );
    } else {
      await sendMessage(
        chatId,
        "⚠️ Invalid phone number format. Please enter 10 digit number manually."
      );
    }
    ...
    await sendMessage(
      chatId,
      "Please send your mobile number to register for OTP services if havn't yet done.",
      {
        keyboard: [
          [{ text: "📞 Send My Phone Number", request_contact: true }],
        ],
